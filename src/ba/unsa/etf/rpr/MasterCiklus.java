...
public class MasterCiklus extends Ciklus {
    ...
    public void dodajSemestar(MasterSemestar ms) throws IllegalArgumentException, ArrayStoreException {
        if (getMaster().contains(ms))
            throw new IllegalArgumentException(ms.ispisiSemestar() + " je vec dodan u ovaj ciklus.");
        else if (getMaster().size() == this.getTrenutniBrojSemestara())
            throw new ArrayStoreException(ms.ispisiSemestar() + " nije dodan, jer je popunjen kapacitet ovog ciklusa.");
        else {
            getMaster().add(ms);
